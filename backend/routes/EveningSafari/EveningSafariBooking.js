...
router.get('/',async (req,res)=>{
    res.send("EveningSafariBooking Endpoint")
})
router.post('/createBooking', async (req,res)=>{
    ...
    // const TimeCheckDocs = await EveningSafariModel.find(query2);
    ...
    const newBooking = new EveningSafariModel(BookingData)
    try{
...
router.get('/SearchByBookingID',async(req,res)=>{
    ...
    if(BookingID){
            try{
                EveningSafariModel.findById(BookingID,function(err,docs){
                if(docs){
...
if(UserIDin==null){
    ...
    EveningSafariModel.find({userID : UserIDin},function (err,docs){
        if(docs){
            res.status(200).json(docs);
        }else{
