...
router.post('/createBooking', async (req,res)=>{
    ...
    // const TimeCheckDocs = await DesertSafariModel.find(query2);
    ...
    const newBooking = new DesertSafariModel(BookingData)
    try{
...
router.get('/SearchByBookingID',async(req,res)=>{
    ...
    if(BookingID){
            try{
                DesertSafariModel.findById(BookingID,function(err,docs){
                if(docs){
...
if(UserIDin==null){
    ...
    DesertSafariModel.find({userID : UserIDin},function (err,docs){
        if(docs){
            res.status(200).json(docs);
        }else{
